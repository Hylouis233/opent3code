...
import { ClaudeAI, MCode, OpenAI, ZCode } from "../Icons";
...
function ImportRowMeta({
  ...
  readonly sources: ReadonlyArray<"claudeAgent" | "codex" | "mcode" | "zcode"> | null;
  ...
  return (
    <span className="ml-auto grid shrink-0 grid-cols-[1rem_1rem_1rem_1rem_2.5rem_2.25rem] items-center gap-x-1 text-xs text-muted-foreground tabular-nums">
      ...
      <span className="flex size-4 items-center justify-center">
        {sources?.includes("mcode") ? <MCode className="size-3" aria-label="MiniMax Code" /> : null}
      </span>
      <span className="flex size-4 items-center justify-center">
        {sources?.includes("zcode") ? <ZCode className="size-3" aria-label="ZCode" /> : null}
      </span>
