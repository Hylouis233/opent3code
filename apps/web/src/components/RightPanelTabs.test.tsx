...
describe("RightPanelTabs audio indicator", () => {
  ...
  const cases = [
    ...
    { audible: true, audioMuted: false, label: "静音 Local site" },
    { audible: true, audioMuted: true, label: "取消静音 Local site" },
  ] as const;
  ...
  it.each(cases)("audible=$audible muted=$audioMuted", ({ audible, audioMuted, label }) => {
    ...
    if (label === null) {
      expect(html).not.toContain("静音 Local site");
      expect(html).not.toContain("取消静音 Local site");
    } else {
