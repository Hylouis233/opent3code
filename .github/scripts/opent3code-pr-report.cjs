...
function matchingPulls(pulls, run, fullName) {
  return pulls.filter((pull) => pull.state === "open" &&
    pull.base.repo.full_name === fullName && pull.head.sha === run.head_sha &&
...
function previewBody({ fullName, sha, runId, conclusion, artifactId }) {
  if (!/^[a-z0-9_.-]+\/[a-z0-9_.-]+$/i.test(fullName) || !/^[a-f0-9]{40}$/i.test(sha) ||
...
async function publishPreview({ github, context, core }) {
  ...
  if (run.event !== "pull_request" || run.repository?.full_name !== fullName ||
    run.path !== ".github/workflows/desktop-macos-preview.yml") return;
  ...
  if (matchingPulls([pull], run, fullName).length !== 1 ||
    !pull.labels.some((label) => PREVIEW_LABELS.includes(label.name))) return;
  ...
  if (!jobs.some((job) => job.name === "Build preview JS bundle" && job.conclusion !== "skipped")) return;
  const artifacts = await github.paginate(github.rest.actions.listWorkflowRunArtifacts, {
    ...repo, run_id: run.id, per_page: 100,
  });
  const bundles = artifacts.filter((item) => item.name === "opent3code-preview-bundle" && !item.expired);
  const body = previewBody({
    fullName, sha: run.head_sha, runId: run.id, conclusion: run.conclusion,
