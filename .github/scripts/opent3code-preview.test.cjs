...
test("preview rejects production namespaces and metadata links", async () => {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "opent3code-init-"));
  try {
    for (const folder of [".t3", ".dsh", ".minimax"])
      assert.throws(() => initializePreview(path.join(root, folder, "child"), root), /namespace/);
    ...
    // oxlint-disable-next-line t3code/no-global-process-runtime -- Dependency-free Node test checks host symlink support, without loading the application runtime.
