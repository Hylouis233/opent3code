...
const run = {
  repository: { id: 1341460159 },
  head_repository: { id: 1341460159 },
  head_branch: "main",
  head_sha: sha,
...
test("source manifest cannot select a stable release or upstream tag", () => {
  assert.equal(
    validateManifest({ enabled: true, sourceOnly: true, version: "0.1.0-alpha.1" }).tag,
    "opent3code-v0.1.0-alpha.1",
  );
  for (const patch of [
    { enabled: false },
    { sourceOnly: false },
    ...
    assert.throws(() =>
      validateManifest({ enabled: true, sourceOnly: true, version: "0.1.0-alpha.1", ...patch }),
    );
});
test("release requires the exact main push and every applicable CI job", () => {
  ...
  for (const patch of [
    ...
    assert.throws(() => assertRun({ ...run, ...patch }, sha, jobs));
...
test("asset receipt, checksums, extra files and path traversal fail closed", () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(root, names[0]), "source fixture");
    fs.writeFileSync(path.join(root, names[1]), "plugin fixture");
    fs.writeFileSync(
      path.join(root, "RELEASE.json"),
      JSON.stringify({ sha, version, ciRun: 1, sourceOnly: true }),
    );
    const sums =
      names
        .map(
    ...
    assert.throws(() => verifyPayload(root, version, sha, 1));
    fs.unlinkSync(path.join(root, "extra"));
    fs.writeFileSync(path.join(root, "SHA256SUMS"), sums.replace(names[0], "../secret"));
    assert.throws(() => verifyPayload(root, version, sha, 1));
    fs.writeFileSync(path.join(root, "SHA256SUMS"), sums);
    fs.appendFileSync(path.join(root, names[0]), "tampered");
    assert.throws(() => verifyPayload(root, version, sha, 1));
  } finally {
