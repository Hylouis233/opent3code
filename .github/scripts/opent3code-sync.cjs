...
function protectedPath(path) {
  return (
    /^(README\.md|CONTRIBUTING\.md|AGENTS\.md|LICENSE|NOTICE)$/.test(path) ||
...
function reviewRequired(files) {
  ...
  return (
    ...
    files.some(
      (file) =>
        protectedPath(file.filename) ||
        (file.previous_filename && protectedPath(file.previous_filename)),
    )
  );
}
...
function assertMergeable(expected, pr, branchSha, validationSucceeded) {
  ...
  if (
    ...
    pr.merge_commit_sha !== expected.merge ||
    branchSha !== expected.base
  ) {
...
async function prepare({ github, context, core }) {
  ...
  const { data: diff } = await github.rest.repos.compareCommits({
    ...context.repo,
    base: baseSha,
    head: candidate.merge_commit_sha,
  });
  ...
  for (const [name, value] of Object.entries({
    ...
    head,
    base: baseSha,
    merge: candidate.merge_commit_sha,
    auto_merge: !manual,
  }))
    core.setOutput(name, String(value));
  await core.summary
    .addRaw(
      `${pr.html_url}\n\nCandidate to validate: ${candidate.merge_commit_sha}\n\n${manual ? "Manual review required: protected paths or incomplete comparison." : "Eligible for validation; not merged yet."}`,
    )
    .write();
}
...
async function merge({ github, context, core, expected }) {
  ...
  if (reviewRequired(diff.files))
    throw new Error("Protected files or incomplete diff require review.");
  ...
  if (status.total_count > 0 && status.state !== "success")
    throw new Error("Commit status is not successful.");
