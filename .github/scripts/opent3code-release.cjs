...
function validateManifest(value) {
  if (
    ...
    !/^0\.1\.0-alpha\.[1-9][0-9]*$/.test(value.version || "")
  )
    throw Error("Only explicitly enabled source Alpha releases are allowed.");
...
function assertRun(run, sha, jobs) {
  if (
    ...
    run.head_repository?.id !== REPO_ID ||
    ...
    throw Error("Not a successful exact-main CI run.");
  for (const name of [
    "PR Gate",
    "OpenT3Code Maintenance",
    "Check",
    "Test",
    "Test Server 1",
    "Test Server 2",
    ...
    if (
      ...
      throw Error(`Required job is missing or unsuccessful: ${name}`);
  }
}
async function textAt(github, repo, file, ref) {
  const { data } = await github.rest.repos.getContent({ ...repo, path: file, ref });
  if (data.type !== "file" || data.encoding !== "base64" || data.size > 100000)
    throw Error("Invalid trusted release input.");
...
async function prepare({ github, context, core }) {
  ...
  if (!run || context.eventName !== "workflow_run")
    throw Error("Only completed CI events are accepted.");
  await verify(github, context.repo, run.id, run.head_sha);
  const manifest = validateManifest(
    JSON.parse(await textAt(github, context.repo, ".github/opent3code-release.json", run.head_sha)),
  );
  try {
    ...
    if (existing.draft)
      throw Error("A draft with this tag already exists; inspect it rather than overwriting it.");
...
function verifyPayload(root, version, sha, runId) {
  const names = [
    `opent3code-source-${version}.zip`,
    `opent3code-dsh-plugin-${version}.tgz`,
    "RELEASE.json",
    "SHA256SUMS",
  ];
  ...
  for (const name of names) {
    ...
    if (!stat.isFile() || stat.size === 0 || stat.size > 250000000)
  ...
  if (
    ...
    receipt.ciRun !== Number(runId) ||
    receipt.sourceOnly !== true
  )
    throw Error("Release receipt mismatch.");
  ...
  for (const row of rows) {
    ...
    const digest = crypto
      .createHash("sha256")
      .update(fs.readFileSync(path.join(root, match[2])))
      .digest("hex");
...
async function publish({ github, context, core, sha, version, runId, root }) {
  ...
  const manifest = validateManifest(
    JSON.parse(await textAt(github, context.repo, ".github/opent3code-release.json", sha)),
  );
  ...
  try {
    const { data: tag } = await github.rest.git.getRef({
      ...context.repo,
      ref: `tags/${manifest.tag}`,
    });
    if (tag.object.type !== "commit" || tag.object.sha !== sha)
      throw Error("Existing tag does not point at this candidate.");
  } catch (error) {
    if (error.status !== 404) throw error;
  ...
  const { data: release } = await github.rest.repos.createRelease({
    ...
    name: `OpenT3Code ${version} — source preview`,
    body: `${MARKER}\n${notes}\n\nSource: \`${sha}\`. Validated by CI run ${runId}.`,
    draft: true,
    prerelease: true,
    make_latest: "false",
  });
      ...
      (name) =>
        !assets.some(
          (asset) =>
            ...
            asset.size === fs.statSync(path.join(root, name)).size,
        ),
    )
  )
    throw Error("Uploaded assets are incomplete; draft preserved.");
