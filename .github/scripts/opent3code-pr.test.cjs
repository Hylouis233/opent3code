...
for (const [lines, expected] of [[0, "XS"], [9, "XS"], [10, "S"], [29, "S"], [30, "M"], [99, "M"], [100, "L"], [499, "L"], [500, "XL"], [999, "XL"], [1000, "XXL"]]) {
  test(`upstream size boundary ${lines}`, () => {
    assert.equal(classifySize([{ filename: "src/a.ts", additions: lines, deletions: 0 }]).label, `size:${expected}`);
  });
}
...
test("mixed PR excludes tests but test-only PR still counts them", () => {
  ...
  assert.equal(classifySize([tests, { filename: "src/a.ts", additions: 10, deletions: 0 }]).label, "size:S");
});
test("renaming production code into tests is not a free exclusion", () => {
  assert.equal(classifySize([{ filename: "tests/a.ts", previous_filename: "src/a.ts", additions: 100, deletions: 0 }]).nonTest, 100);
});
...
test("a stale PR head cannot update labels", async () => {
  ...
  await syncLabel({
    github: { rest: {
      ...
      issues: { addLabels: async () => calls.push("add"), removeLabel: async () => calls.push("remove") },
    } },
    repo: { owner: "owner", repo: "repo" }, pull: { number: 1, head: { sha: "old" } },
    label: "size:S", managed: ["size:S", "size:M"], core: { info() {} },
  });
...
test("only managed stale labels are removed", async () => {
  ...
  await syncLabel({
    github: { rest: {
      pulls: { get: async () => ({ data: { state: "open", head: { sha: "same" }, labels: [{ name: "bug" }, { name: "size:M" }] } }) },
      issues: { addLabels: async (arg) => calls.push(arg.labels), removeLabel: async (arg) => calls.push(arg.name) },
    } },
    repo: { owner: "owner", repo: "repo" }, pull: { number: 1, head: { sha: "same" } },
    label: "size:S", managed: ["size:S", "size:M"], core: { info() {} },
  });
