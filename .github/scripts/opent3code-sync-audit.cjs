...
function git(repo, args, allowed = [0]) {
  ...
  const result = spawnSync(
    "git",
    ["-c", "core.hooksPath=/dev/null", "-c", "protocol.ext.allow=never", "-c", "protocol.file.allow=never", "-c", "submodule.recurse=false", "-C", repo, ...args],
    { env, maxBuffer: 128 * 1024 * 1024, timeout: 300000 },
  );
  if (result.error || !allowed.includes(result.status)) {
    throw Error(`Git ${args[0]} failed: ${result.error?.message || decoder.decode(result.stderr).slice(0, 4000)}`);
  }
...
function parseDiff(text) {
  ...
  for (let i = 0; i < fields.length; i += 2) {
    ...
    if (!match || !filename || seen.has(filename)) throw Error("Invalid or duplicate raw diff record.");
    seen.add(filename);
    files.push({ filename, status: match[5], oldMode: match[1], newMode: match[2], oldBlob: match[3], newBlob: match[4] });
  }
  return files;
}
...
function sensitive(filename) {
  return /^(README\.md|CONTRIBUTING\.md|AGENTS\.md|LICENSE|NOTICE)$/.test(filename) ||
    filename.startsWith(".github/") || filename.startsWith("integrations/dsh-opent3code/") ||
    filename.includes("opent3code") || filename.includes("ExternalAcp") ||
    filename.endsWith("provider/builtInDrivers.ts") || filename.endsWith("providerDriverMeta.ts");
}
function analyze(repo, base, head) {
  ...
  for (const sha of [base, head]) {
    if (git(repo, ["rev-parse", `${sha}^{commit}`]).text.trim() !== sha) throw Error("Commit identity mismatch.");
  }
  ...
  const diff = (from, to) => parseDiff(git(repo, ["diff", "--raw", "--no-abbrev", "--no-renames", "--no-ext-diff", "--no-textconv", "-z", from, to, "--"]).text);
  ...
  const overlap = forkFiles.filter((file) => upstreamNames.has(file.filename)).map((file) => file.filename);
  const output = git(repo, ["merge-tree", "--write-tree", "--name-only", "--no-messages", "-z", base, head], [0, 1]);
  ...
  return {
    format: 1, diagnosticOnly: true, validationSucceeded: false, mergeAuthorized: false,
    base, head, mergeBase: ancestor,
    ...
    renameDetection: false, upstreamFiles, forkFiles, overlap,
    sensitiveUpstreamFiles: upstreamFiles.filter((file) => sensitive(file.filename)).map((file) => file.filename),
    upstreamPathGroups: groups, mergeComputation: merge,
...
function summary(report) {
  ...
  return [
    ...
    `Fork: ${report.base}`, `Upstream: ${report.head}`, `Merge base: ${report.mergeBase}`, "",
    ...
    "## Conflicts", `<pre>${escape(JSON.stringify(report.mergeComputation.conflicts, null, 2))}</pre>`, "",
    "## Largest upstream path groups",
    `<pre>${escape(JSON.stringify(Object.entries(report.upstreamPathGroups).sort((a, b) => b[1] - a[1]).slice(0, 20), null, 2))}</pre>`, "",
    "## Fork/upstream overlap", `<pre>${escape(JSON.stringify(report.overlap, null, 2))}</pre>`, "",
...
function main() {
  ...
  if (!root || !path.isAbsolute(root) || fs.existsSync(root)) throw Error("AUDIT_ROOT must be a new absolute directory.");
  ...
  git(repo, ["fetch", "--quiet", "--no-tags", "--no-recurse-submodules", "https://github.com/Hylouis233/opent3code.git", base]);
  git(repo, ["fetch", "--quiet", "--no-tags", "--no-recurse-submodules", "https://github.com/pingdotgg/t3code.git", head]);
