...
const run = { head_sha: sha, head_branch: "feature", head_repository: { full_name: "contributor/fork" } };
const pull = { state: "open", base: { repo: { full_name: "owner/repo" } }, head: { sha, ref: "feature", repo: { full_name: "contributor/fork" } } };
test("fork PR resolution requires matching repository, branch, SHA and open state", () => {
  ...
  assert.equal(matchingPulls([pull], { ...run, head_repository: { full_name: "attacker/fork" } }, "owner/repo").length, 0);
});
test("successful artifacts are clearly unsigned and not release binaries", () => {
  const body = previewBody({ fullName: "owner/repo", sha, runId: 1, conclusion: "success", artifactId: 2 });
...
test("untrusted identifiers cannot inject URLs or markup into comments", () => {
  for (const input of [{ fullName: "owner/repo](bad)" }, { sha: "bad" }, { runId: -1 }, { artifactId: "2" }]) {
    assert.throws(() => previewBody({ fullName: "owner/repo", sha, runId: 1, conclusion: "success", ...input }));
  }
});
