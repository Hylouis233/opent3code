...
/**
 * The read-file tool title is display copy generated by toolActivity and is
 * localized; match every shipped spelling instead of one locale's string.
 */
const READ_FILE_TOOL_TITLES = new Set(["read file", "读取文件"]);

function isReadFileToolTitle(toolTitle: string | undefined): boolean {
  const normalized = toolTitle?.trim().toLowerCase();
  return normalized !== undefined && READ_FILE_TOOL_TITLES.has(normalized);
}
...
export function toolGroupAction(entry: WorkLogPresentationEntry): ToolGroupAction {
  ...
  if (
    ...
    (entry.itemType === "dynamic_tool_call" && isReadFileToolTitle(entry.toolTitle))
  ) {
    return "read";
  }
  if (
